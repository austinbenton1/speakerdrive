  ...
  RETURN QUERY EXECUTE format('
    ...
    cte_events AS (
      SELECT
        leads.*,
        CASE
          -- First priority: URL exists for this event
          WHEN EXISTS (
            SELECT 1 FROM leads l2 
            WHERE l2.event_name = leads.event_name 
            AND l2.unlock_type = ''Unlock Event URL''
          ) AND leads.unlock_type = ''Unlock Event URL''
          THEN 2
          -- Second priority: First tier email patterns if no URL exists
          WHEN NOT EXISTS (
            SELECT 1 FROM leads l2 
            WHERE l2.event_name = leads.event_name 
            AND l2.unlock_type = ''Unlock Event URL''
          )
          AND leads.unlock_type = ''Unlock Event Email''
          AND (
          ...
          THEN 2
          -- Third priority: Second tier email patterns if no URL or first tier exists
          WHEN NOT EXISTS (
            SELECT 1 FROM leads l2 
            WHERE l2.event_name = leads.event_name 
            AND (
